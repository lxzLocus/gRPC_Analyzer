...
// @ts-ignore: JS モジュールのため型チェックを無視
import { findAllAndMergeProjectRoots } from '../modules/editFilePathStructure.js';
/*config*/
        ...
        for (const category of categoryDirs) {
            ...
            for (const pullRequestTitle of titleDirs) {
                ...
                for (const changedProtoFile of changedProtoFiles) {
                    const protoInfo = allProtoContentList.proto_files?.find((proto) => proto.path === changedProtoFile);
                    if (protoInfo) {
                ...
                const relevantProtoFiles = [];
                const otherProtoFilePaths = [];
                if (allProtoContentList.proto_files) {
                    for (const protoInfo of allProtoContentList.proto_files) {
                        ...
                        const isImported = importedProtoFiles.has(normalizedPath) ||
                            ...
                            Array.from(importedProtoFiles).some(importPath => normalizedPath.endsWith(importPath) || filePath.endsWith(importPath));
...
export { main };
//# sourceMappingURL=generatePrompt_new.js.map
...
%%_Fin_%%`,
     ...
     */
    injectMockIntoController(controller) {
        // OpenAIClientのメソッドをモックに置き換え
        const originalOpenAIClient = controller.openAIClient;
        if (originalOpenAIClient) {
            // setMockResponseメソッドを使用してモックを設定
            originalOpenAIClient.setMockResponse = (response) => {
            ...
            originalOpenAIClient.fetchOpenAPI = async (messages) => {
...
//# sourceMappingURL=mockLLMTestRunner.js.map
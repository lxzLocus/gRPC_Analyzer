#!/usr/bin/env node
/**
 * filtered_fewChangedをfiltered_confirmedにコピーし、
 * 03_fileChanges.txt内の.protoと*.pb.*ファイルを
 * merge/commit_snapshotからpremergeへ上書きコピー
 * 
 * これにより、premergeはprotoとスタブが更新され、
 * 手書きコードだけ修正すればいい状態になる
 */
...
function copyDirectory(source: string, target: string): void {
    console.log(`📂 コピー中: ${source} → ${target}`);
...
function main() {
    ...
    console.log('🚀 Dataset Copy and Update Tool');
    console.log('='.repeat(80));
    console.log();
    
    // Step 1: ディレクトリコピー
    console.log('📋 Step 1: ディレクトリコピー');
    ...
    // Step 2: protoとスタブファイルの更新
    console.log('📋 Step 2: Proto/Stubファイルの更新');
    ...
    // 詳細レポートをJSON出力
    const reportPath = '/app/output/dataset_update_report.json';

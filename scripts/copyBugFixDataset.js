...
if (fs.existsSync(TARGET_DIR)) {
    console.log('  ⚠️  既存のディレクトリが存在します');
    const answer = 'y'; // 自動的に上書き
    if (answer === 'y') {
        console.log('  🗑️  既存ディレクトリを削除中...');
        ...
        try {
            ...
            for (const subDir of subDirs) {
                ...
                if (fs.existsSync(prPath)) {
                    ...
                    // rsync でコピー（シンボリックリンク保持、進捗表示なし）
                    execSync(`rsync -a "${prPath}/" "${targetPrPath}/"`, { stdio: 'pipe' });

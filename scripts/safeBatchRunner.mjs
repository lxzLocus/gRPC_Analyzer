...
class SafeBatchRunner {
    ...
    async processSinglePullRequest(repositoryName, category, pullRequestTitle, pullRequestPath, premergeDir) {
        ...
        for (let retry = 0; retry <= maxRetries; retry++) {
            try {
                ...
                // 実際のLLMFlowControllerを使用
                const LLMFlowController = await import('../dist/js/modules/llmFlowController.js');
                controller = new LLMFlowController.default(premergeDir);
                
                // エラーハンドリングを強化した処理実行
                const result = await this.runControllerWithEnhancedErrorHandling(controller, repositoryName, category, pullRequestTitle);
                
                // ログファイルベースで実際の処理結果を確認
                ...
                if (controller && typeof controller.cleanup === 'function') {
                    ...
                    // LLMFlowControllerには特定のcleanupメソッドがないため、リソースの明示的な解放
                    try {
                        controller.currentMessages = [];
                        controller.context = {};
                    } catch (cleanupError) {
                        console.warn(`⚠️  Controller resource cleanup failed: ${cleanupError}`);
                    }
                }
     ...
     */
    async runControllerWithEnhancedErrorHandling(controller, repositoryName, category, pullRequestTitle) {
        try {
            // 実行前にメモリ状況をチェック
            ...
            // 実際の処理実行（LLMFlowControllerのrunメソッドを呼び出し）
            ...
            // LLMFlowControllerは戻り値がないため、実行完了をもって成功とする
            return { success: true };
     ...
     * JSONパースエラーの詳細分析
     */
    async analyzeJsonParseError(repositoryName, category, pullRequestTitle, error) {
        try {
            const logDir = path.join('/app/log', repositoryName, category, pullRequestTitle);
            if (!fs.existsSync(logDir)) {
...
const gracefulShutdown = (signal) => {
    ...
    if (currentRunner) {
        try {
            // 現在進行中の処理の情報を保存
            const currentStats = currentRunner.stats;

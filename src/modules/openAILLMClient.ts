...
export class OpenAILLMClient implements LLMClient {
    ...
    constructor(config: Config, apiKey?: string) {
        ...
        console.log(`🤖 OpenAILLMClient: Using model: ${this.config.get('llm.model', this.config.get('openai.model', process.env.OPENAI_MODEL || 'gpt-4.1'))}`);
        console.log(`🔧 Model priority: llm.model > openai.model > OPENAI_MODEL > default(gpt-4.1)`);
    ...
    async generateContent(request: LLMRequest): Promise<LLMResponse> {
        ...
        try {
            const model = request.model || this.config.get('llm.model', this.config.get('openai.model', process.env.OPENAI_MODEL || 'gpt-4.1'));
